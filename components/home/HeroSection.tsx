...
// Valores padrão (fallback quando banco não tem dados ainda)
const DEFAULTS = {
  badge: 'Gráfica · Design · Digital',
  line1: 'Do físico ao',
  highlight: 'digital.',
  line3: 'Em um só lugar.',
  subtitle: 'Gráfica, design e tecnologia integrados para construir a identidade completa da sua marca — do cartão de visita à loja virtual.',
  cta1Label: 'Solicitar Orçamento Grátis →',
  cta1Link: '#contato',
  cta2Label: 'Ver Portfólio',
  cta2Link: '/portfolio',
  stats: '+200 projetos entregues',
};

type HeroData = typeof DEFAULTS;

export default function HeroSection({ data }: { data?: Partial<HeroData> | null }) {
      ...
      <div className="container-custom relative z-10 grid lg:grid-cols-2 gap-12 items-center py-20">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-2xl">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-[72px] leading-[1.1] font-titulo font-extrabold text-t-primary mb-6 tracking-tight">
            <motion.div variants={itemVariants}>{d.line1}</motion.div>
            <motion.div variants={itemVariants} className="font-display italic text-[80px] text-laranja leading-[0.9] my-2">
              {d.highlight}
            </motion.div>
            <motion.div variants={itemVariants}>{d.line3}</motion.div>
          </h1>
          ...
          <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 mb-12">
            <Link href={d.cta1Link} className="bg-laranja text-white px-8 py-4 rounded-full font-medium text-center hover:bg-[#D4651A] hover:scale-105 active:scale-95 transition-all shadow-cor">
              {d.cta1Label}
            </Link>
            <Link href={d.cta2Link} className="bg-transparent border-2 border-roxo text-roxo px-8 py-4 rounded-full font-medium text-center hover:bg-roxo-light hover:scale-105 active:scale-95 transition-all">
              {d.cta2Label}
            </Link>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="flex items-center gap-4">
            ...
            <div className="text-sm text-t-muted font-medium">
              {d.stats} · <span className="text-ambar">★★★★★</span>
            </div>
          </motion.div>
        </motion.div>

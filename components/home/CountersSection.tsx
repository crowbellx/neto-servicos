...
type CounterItem = { value: number; suffix: string; label: string };

const DEFAULTS: CounterItem[] = [
...
const Counter = ({ end, suffix = '', label }: { end: number; suffix?: string; label: string }) => {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center justify-center text-center">
      ...
      <div className="text-sm text-t-white-70 font-corpo max-w-[120px] leading-snug">{label}</div>
    </div>
  );
};

export default function CountersSection({ data }: { data?: { items?: CounterItem[] } | null }) {
  const counters = data?.items?.length ? data.items : DEFAULTS;
    ...
    <section id="numeros" className="bg-grafite py-16 lg:py-24 relative overflow-hidden">
      <div className="container-custom grid grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
        {counters.map((counter, index) => (
          ...
          >
            <Counter end={counter.value} suffix={counter.suffix} label={counter.label} />
          </motion.div>
        ))}
      </div>
    </section>
